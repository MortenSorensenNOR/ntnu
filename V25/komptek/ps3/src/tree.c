...
static node_t* constant_fold_operator(node_t* node)
{
    ...
    bool all_children_num_litterals = true;
    for (size_t i = 0; i < node->n_children; i++)
        if (node->children[i]->type != NUMBER_LITERAL)
            all_children_num_litterals = false;

    if (!all_children_num_litterals)
        return node;

    // Do constant folding 
    int64_t result = 0;
    if (!strcmp(node->data.operator, "+")) {
        assert(node->n_children == 2);
        result = node->children[0]->data.number_literal + node->children[1]->data.number_literal;
    } else if (!strcmp(node->data.operator, "-")) {
        if (node->n_children == 2) {
            result = node->children[0]->data.number_literal - node->children[1]->data.number_literal;
        } else if (node->n_children == 1) {
            result = -node->children[0]->data.number_literal;
        } else {
            printf("Error: Unexpected operator -. Got %ld terms, expected 1 or 2\n", node->n_children);
        }
    } else if (!strcmp(node->data.operator, "*")) {
        assert(node->n_children == 2);
        result = node->children[0]->data.number_literal * node->children[1]->data.number_literal;
        ...
        printf("Error: Unimplemented operator (%s)\n", node->data.operator);
    }
