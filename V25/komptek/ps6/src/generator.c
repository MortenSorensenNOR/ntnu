...
static void generate_if_statement(node_t* statement)
{
    ...
    int current_if_statement_num = if_statement_num;
    if_statement_num++;

    if (statement->n_children == 2) {
        // IF
        EMIT("je ENDIF%d", current_if_statement_num);
        generate_statement(statement->children[1]);
        EMIT("ENDIF%d:", current_if_statement_num);
    } else {
        // IF-ELSE
        EMIT("je ELSE%d", current_if_statement_num);
        generate_statement(statement->children[1]);
        EMIT("jmp ENDIF%d", current_if_statement_num);
        EMIT("ELSE%d:", current_if_statement_num);
        generate_statement(statement->children[2]);
        EMIT("ENDIF%d:", current_if_statement_num);
    }
}

static int while_statement_num = 0;
static int curr_while_loop = 0;
static void generate_while_statement(node_t* statement)
{
    ...
    int prev_curr_while = curr_while_loop;

    int current_while_statement = while_statement_num;
    curr_while_loop = while_statement_num;
    while_statement_num++;

    // Start
    EMIT("WHILE%d:", current_while_statement);

    // Expression
    assert(statement->n_children == 2);
    generate_expression(statement->children[0]);
    CMPQ("$0", RAX);
    EMIT("je ENDWHILE%d", current_while_statement);

    // While body
    generate_statement(statement->children[1]);
    EMIT("jmp WHILE%d", current_while_statement);

    // End
    EMIT("ENDWHILE%d:", current_while_statement);
    curr_while_loop = prev_curr_while;
}
...
static void generate_break_statement()
{
    ...
    EMIT("jmp ENDWHILE%d", curr_while_loop);
}
